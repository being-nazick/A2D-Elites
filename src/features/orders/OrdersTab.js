...
export default function OrdersTab({ data, onAdd, onEdit, onDelete, onMarkDelivered, onMarkPaid, OrderRow }) {
  ...
  return <div>
    ...
    <div style={{ padding: "10px 16px 0", display: "flex", flexDirection: "column", gap: 8 }}>{filtered.length === 0 && <div style={{ textAlign: "center", color: C.inkMute, fontSize: 13, padding: "30px 0" }}>No orders match your filters.</div>}{filtered.map((order) => <div key={order.id}><div style={{ fontSize: 10.5, color: C.inkMute, marginBottom: 3, fontFamily: "'JetBrains Mono',monospace" }}>{fmtDateShort(order.orderDate)}</div><OrderRow order={order} actions={{ markDelivered: onMarkDelivered, markPaid: onMarkPaid, edit: onEdit, del: onDelete }} /></div>)}</div><div style={{ height: 12 }} />
  </div>;
}
